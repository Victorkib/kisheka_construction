    ...
    <div className="ds-bg-surface rounded-lg shadow p-6 border-2 ds-border-subtle">
            ...
            <label
              ...
              className={`flex items-start gap-3 p-3 border-2 rounded-lg cursor-pointer transition ${
                reportType === type.value
                  ...
                  : 'ds-border-subtle hover:ds-border-subtle'
              }`}
            >
              <input
                type="radio"
              ...
              <div className="flex-1">
                <div className="font-medium ds-text-primary">{type.label}</div>
                <div className="text-xs ds-text-secondary mt-1">{type.description}</div>
              </div>
            </label>
          ))}
        </div>
      ...
      {reportType === 'financial' && (
        <div className="mb-6 p-4 ds-bg-surface-muted rounded-lg">
          <h4 className="font-semibold ds-text-primary mb-3">Report Options</h4>
            ...
            <div className="pt-2 border-t">
              <label className="flex items-center gap-2 mb-2">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 ds-border-subtle rounded"
                />
                <span className="text-sm ds-text-secondary">Filter by Date Range</span>
              </label>
              {options.dateRange.enabled && (
                <div className="grid grid-cols-2 gap-3 ml-6">
                  <div>
                    <label className="block text-xs ds-text-secondary mb-1">Start Date</label>
                    <input
                      type="date"
                      ...
                      className="w-full px-2 py-1 text-sm border ds-border-subtle rounded"
                    />
                  </div>
                  <div>
                    <label className="block text-xs ds-text-secondary mb-1">End Date</label>
                    <input
                      type="date"
      ...
      {reportData && (
        <div className="border-t pt-6">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-semibold ds-text-primary">Generated Report</h4>
            <div className="flex gap-2">
              <button
                onClick={handleExportJSON}
                className="px-3 py-1.5 text-sm ds-bg-surface-muted hover:ds-bg-surface-muted ds-text-secondary rounded-lg transition"
              >
                Export JSON
              </button>
              <button
          ...
          {/* Report Preview */}
          <div className="ds-bg-surface-muted rounded-lg p-4 max-h-96 overflow-y-auto">
            {reportType === 'summary' && reportData.categories && (
              <div className="space-y-3">
                <h5 className="font-semibold ds-text-primary">Cost Category Summary</h5>
                <div className="grid grid-cols-5 gap-2 text-xs font-semibold ds-text-secondary border-b pb-2">
                  <div>Category</div>
                ...
                <div className="grid grid-cols-5 gap-2 text-sm font-semibold ds-text-primary pt-2 border-t">
                  <div>TOTAL</div>
            ...
            {reportType === 'phases' && reportData.phases && (
              <div className="space-y-3">
                <h5 className="font-semibold ds-text-primary">Phase-Wise Breakdown</h5>
                {reportData.phases.map((phase) => (
                  <div key={phase.phaseId} className="border ds-border-subtle rounded-lg p-3">
                    <div className="font-semibold ds-text-primary mb-2">
                      {phase.phaseName} {phase.phaseCode ? `(${phase.phaseCode})` : ''}
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      <div>
                        <span className="ds-text-secondary">Budget: </span>
                        <span className="font-medium">{formatCurrency(phase.budget)}</span>
                      </div>
                      <div>
                        <span className="ds-text-secondary">Spending: </span>
            ...
            {reportType === 'financial' && reportData.budgets && (
              <div className="space-y-4">
                <div>
                  <h5 className="font-semibold ds-text-primary mb-2">Summary</h5>
                  <div className="grid grid-cols-4 gap-3 text-sm">
                    <div>
                      <span className="ds-text-secondary">Total Budget: </span>
                      <span className="font-medium">{formatCurrency(reportData.summary.totalBudget)}</span>
                    </div>
                    <div>
                      <span className="ds-text-secondary">Total Spending: </span>
                      <span className="font-medium">{formatCurrency(reportData.summary.totalSpending)}</span>
                    </div>
                    <div>
                      <span className="ds-text-secondary">Total Remaining: </span>
                ...
                <div>
                  <h5 className="font-semibold ds-text-primary mb-2">Cost Categories</h5>
                  {Object.entries(reportData.budgets).map(([cat, data]) => (
                    <div key={cat} className="mb-2 p-2 ds-bg-surface rounded border ds-border-subtle">
                      <div className="font-medium ds-text-primary mb-1">{cat.toUpperCase()}</div>

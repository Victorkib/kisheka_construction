...
export function RecommendationsPanel({ projectId }) {
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      case 'high':
        return 'border-red-400/60 bg-red-50';
      case 'medium':
        return 'border-yellow-400/60 bg-yellow-50';
      case 'low':
        return 'border-blue-400/60 bg-blue-50';
      default:
        return 'ds-border-subtle ds-bg-surface-muted';
    }
  };
  ...
  if (!recommendations || !recommendations.recommendations || recommendations.recommendations.length === 0) {
    return (
      <div className="ds-bg-surface rounded-lg shadow p-6 border-2 ds-border-subtle">
        <h3 className="text-lg font-semibold ds-text-primary mb-4">Recommendations</h3>
    ...
    <>
      <div className="ds-bg-surface rounded-lg shadow p-6 border-2 ds-border-subtle">
        ...
        {recommendations.summary && (
          <div className="grid grid-cols-4 gap-4 mb-6 p-4 ds-bg-surface-muted rounded-lg">
            <div>
              <p className="text-xs ds-text-secondary mb-1">Total</p>
              <p className="text-sm font-semibold ds-text-primary">
                {recommendations.summary.total}
              </p>
            </div>
            <div>
              <p className="text-xs ds-text-secondary mb-1">Warnings</p>
              <p className="text-sm font-semibold text-red-600">
                {recommendations.summary.byType.warning}
              </p>
            </div>
            <div>
              <p className="text-xs ds-text-secondary mb-1">Info</p>
              <p className="text-sm font-semibold text-blue-600">
                {recommendations.summary.byType.info}
              </p>
            </div>
            <div>
              <p className="text-xs ds-text-secondary mb-1">Actionable</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-semibold ds-text-primary">{rec.title}</h4>
                    <span className="px-2 py-1 text-xs font-semibold rounded ds-bg-surface">
                      {rec.priority.toUpperCase()}
                    </span>
                  </div>
                  <p className="text-sm ds-text-secondary mb-2">{rec.message}</p>
                  <p className="text-sm ds-text-secondary mb-3">{rec.action}</p>
                  {rec.actionable && (
                    <button

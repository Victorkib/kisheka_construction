        ...
        {[1, 2, 3].map((item) => (
          <Card key={item} className="p-6">
            <div className="animate-pulse space-y-3">
              ...
              <div className="h-6 ds-bg-surface-muted rounded w-2/3"></div>
            </div>
          </Card>
        ))}
      </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {metrics.map((metric) => (
        <Card key={metric.label} className="p-6">
          <p className="text-sm ds-text-muted">{metric.label}</p>
          <p className="mt-2 text-2xl font-semibold ds-text-primary">

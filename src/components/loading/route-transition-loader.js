...
export function RouteTransitionLoader({ isLoading, message = 'Loading...' }) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 ds-bg-surface bg-opacity-90 backdrop-blur-sm border-b ds-border-subtle">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex items-center justify-center gap-3">
          <LoadingSpinner size="sm" color="blue-600" />
          <span className="text-sm ds-text-secondary">{message}</span>
        </div>
      </div>
    </div>
  );

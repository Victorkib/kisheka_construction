  ...
  return (
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold ds-text-primary">{financialSummary.utilizationPercentage.toFixed(1)}%</p>
        ...
        <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              ...
              <p className="text-xl font-bold ds-text-primary">{formatCurrency(budgetAllocation.materials || 0)}</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Labour</p>
              <p className="text-xl font-bold ds-text-primary">{formatCurrency(budgetAllocation.labour || 0)}</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Equipment</p>
              <p className="text-xl font-bold ds-text-primary">{formatCurrency(budgetAllocation.equipment || 0)}</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Subcontractors</p>
              <p className="text-xl font-bold ds-text-primary">{formatCurrency(budgetAllocation.subcontractors || 0)}</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Contingency</p>
              <p className="text-xl font-bold ds-text-primary">{formatCurrency(budgetAllocation.contingency || 0)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-semibold ds-text-primary mb-4">Cost Breakdown</h2>
          
          {/* Materials */}
          <div className="mb-6">
            <h3 className="text-md font-semibold ds-text-primary mb-3">Materials</h3>
            <div className="space-y-2">
              {breakdown.materials.byCategory.length > 0 ? (
                breakdown.materials.byCategory.map((item, index) => (
                  <div key={index} className="flex justify-between items-center py-2 border-b">
                    <div>
                      <p className="font-medium ds-text-primary">{item.category}</p>
                      <p className="text-xs ds-text-muted">{item.count} items</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold ds-text-primary">{formatCurrency(item.totalCost)}</p>
                      <p className="text-xs ds-text-muted">{item.percentage}%</p>
                    </div>
                  </div>
                ))
              ) : (
                <p className="ds-text-muted text-sm">No materials data</p>
              )}
            </div>
          </div>
          ...
          {/* Expenses */}
          <div className="mb-6">
            <h3 className="text-md font-semibold ds-text-primary mb-3">Expenses</h3>
            <div className="space-y-2">
              {breakdown.expenses.byCategory.length > 0 ? (
                ...
                <p className="ds-text-muted text-sm">No expenses data</p>
              )}
            </div>
          </div>
          ...
          {/* Equipment */}
          <div className="mb-6">
            <h3 className="text-md font-semibold ds-text-primary mb-3">Equipment</h3>
            <div className="space-y-2">
              {breakdown.equipment.byType.length > 0 ? (
                breakdown.equipment.byType.map((item, index) => (
                  <div key={index} className="flex justify-between items-center py-2 border-b">
                    <div>
                      <p className="font-medium ds-text-primary">{item.type}</p>
                      <p className="text-xs ds-text-muted">{item.count} items</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold ds-text-primary">{formatCurrency(item.totalCost)}</p>
          ...
          {/* Subcontractors */}
          <div className="mb-6">
            <h3 className="text-md font-semibold ds-text-primary mb-3">Subcontractors</h3>
            <div className="space-y-2">
              {breakdown.subcontractors.byType.length > 0 ? (
                breakdown.subcontractors.byType.map((item, index) => (
                  <div key={index} className="flex justify-between items-center py-2 border-b">
                    <div>
                      <p className="font-medium ds-text-primary">{item.type}</p>
                      <p className="text-xs ds-text-muted">{item.count} contracts</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold ds-text-primary">{formatCurrency(item.totalCost)}</p>
                      <p className="text-xs ds-text-muted">{item.percentage}%</p>
        ...
        {/* Variance Analysis */}
        <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-semibold ds-text-primary mb-4">Variance Analysis</h2>
          <div className="space-y-4">
            {Object.entries(variance.byCategory).map(([category, data]) => (
              <div key={category} className="border rounded-lg p-4">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="font-semibold ds-text-primary capitalize">{category}</h3>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    <p className="ds-text-secondary">Budgeted</p>
                    <p className="font-semibold ds-text-primary">{formatCurrency(data.budgeted)}</p>
                  </div>
                  <div>
        ...
        {trends && (trends.materials.length > 0 || trends.expenses.length > 0) && (
          <div className="ds-bg-surface rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold ds-text-primary mb-4">Cost Trends</h2>
            <div className="space-y-4">
              {trends.materials.length > 0 && (
                <div>
                  <h3 className="text-md font-semibold ds-text-primary mb-2">Materials by Month</h3>
                  <div className="space-y-2">
                    {trends.materials.map((item, index) => (
                      <div key={index} className="flex justify-between items-center py-2 border-b">
                        <span className="text-sm ds-text-secondary">{item.period}</span>
                        <span className="font-semibold ds-text-primary">{formatCurrency(item.totalCost)}</span>
                        <span className="text-xs ds-text-muted">{item.count} items</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {trends.expenses.length > 0 && (
                <div>
                  <h3 className="text-md font-semibold ds-text-primary mb-2">Expenses by Month</h3>
                  <div className="space-y-2">

  ...
  if (loading) {
    return (
      <div className="ds-bg-surface rounded-lg shadow p-4 sm:p-6 mb-6">
        <h2 className="text-base sm:text-lg font-semibold ds-text-primary mb-4">Construction Phases</h2>
        <p className="text-sm ds-text-muted">Loading phases...</p>
      </div>
    );
  }
  ...
  return (
    <div className="ds-bg-surface rounded-lg shadow p-4 sm:p-6 mb-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-2 mb-4">
        <h2 className="text-base sm:text-lg font-semibold ds-text-primary">Construction Phases</h2>
      ...
      {phases.length === 0 ? (
        <div className="text-center py-8">
          <p className="ds-text-muted mb-2">No phases defined for this project</p>
        ...
        <div className="space-y-4">
          ...
          {projectBudget && phases.length > 0 && (() => {
            ...
            return (
              <div className="ds-bg-surface-muted rounded-lg p-3 sm:p-4 mb-4 border ds-border-subtle">
                <h3 className="text-xs sm:text-sm font-semibold ds-text-secondary mb-3">Phase Budget Summary</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                  <div>
                    <p className="text-xs ds-text-secondary mb-1">Project Budget</p>
                    <p className="text-sm font-semibold ds-text-primary">{formatCurrency(projectTotal)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-semibold ${
                      unallocatedBudget < 0 ? 'text-red-600' : unallocatedBudget > 0 ? 'text-yellow-600' : 'ds-text-secondary'
                    }`}>
                      {formatCurrency(unallocatedBudget)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs ds-text-secondary mb-1">Allocation</p>
                    <p className="text-sm font-semibold ds-text-primary">{allocationPercentage.toFixed(1)}%</p>
          ...
          {phases.slice(0, 5).map((phase) => {
            ...
            return (
              <Link
                ...
                className="block border ds-border-subtle rounded-lg p-4 hover:ds-bg-surface-muted transition-colors"
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold ds-text-primary">{phase.phaseName}</h3>
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(phase.status)}`}>
                        {phase.status.replace('_', ' ').toUpperCase()}
                      </span>
                      <span className="text-xs ds-text-muted">{phase.phaseCode}</span>
                    </div>
  ...
  if (loading) {
    return (
      <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold ds-text-primary mb-4">Budget vs Actual</h2>
        <p className="text-sm ds-text-muted">Loading budget variance data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="p-4 border ds-border-subtle rounded-lg">
          <p className="text-xs ds-text-secondary mb-1">Total Budget</p>
          <p className="text-lg font-bold ds-text-primary">{formatCurrency(budgetData.budget.total)}</p>
          <p className="text-xs ds-text-secondary mt-2">Actual</p>
          <p className="text-base font-semibold ds-text-primary">{formatCurrency(budgetData.actual.total)}</p>
          <div className="mt-2">
        ...
        <div className="p-4 border ds-border-subtle rounded-lg">
          <p className="text-xs ds-text-secondary mb-1">Materials</p>
          <p className="text-sm ds-text-muted">Budget: {formatCurrency(budgetData.budget.materials)}</p>
          <p className="text-sm ds-text-muted">Actual: {formatCurrency(budgetData.actual.materials)}</p>
          <div className="mt-2">
        ...
        <div className="p-4 border ds-border-subtle rounded-lg">
          <p className="text-xs ds-text-secondary mb-1">Labour</p>
          <p className="text-sm ds-text-muted">Budget: {formatCurrency(budgetData.budget.labour)}</p>
          <p className="text-sm ds-text-muted">Actual: {formatCurrency(budgetData.actual.labour)}</p>
          <div className="mt-2">
        ...
        <div className="p-4 border ds-border-subtle rounded-lg">
          <p className="text-xs ds-text-secondary mb-1">Contingency</p>
          <p className="text-sm ds-text-muted">Budget: {formatCurrency(budgetData.budget.contingency)}</p>
          <p className="text-sm ds-text-muted">Used: {formatCurrency(budgetData.actual.contingency)}</p>
          <div className="mt-2">
          ...
          return (
            <>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm ds-text-secondary">Budget Utilization</span>
...
function ExpensesSection({ projectId }) {
  ...
  return (
    <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
      ...
      <div className="mb-4">
        <p className="text-sm ds-text-secondary">Total Approved Expenses</p>
        <p className="text-2xl font-bold text-orange-600 mt-1">{formatCurrency(totalAmount)}</p>
      </div>
      {loading ? (
        <p className="text-sm ds-text-muted">Loading...</p>
      ) : expenses.length === 0 ? (
          ...
          {expenses.slice(0, 5).map((expense) => (
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium ds-text-primary">{expense.description || expense.expenseCode}</p>
                ...
                <div className="text-right">
                  <p className="font-semibold ds-text-primary">{formatCurrency(expense.amount, expense.currency)}</p>
...
function InitialExpensesSection({ projectId }) {
  ...
  return (
    <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
      ...
      <div className="mb-4">
        <p className="text-sm ds-text-secondary">Total Initial Expenses</p>
        <p className="text-2xl font-bold text-orange-600 mt-1">{formatCurrency(totalAmount)}</p>
      </div>
      {loading ? (
        <p className="text-sm ds-text-muted">Loading...</p>
        ...
        <div className="space-y-2">
          {initialExpenses.slice(0, 5).map((expense) => (
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium ds-text-primary">{expense.itemName}</p>
                  <p className="text-sm ds-text-secondary leading-normal">{expense.expenseCode}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold ds-text-primary">{formatCurrency(expense.amount)}</p>
...
function ProjectFinancesSection({ projectId }) {
  ...
  return (
    <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold ds-text-primary">Project Finances</h2>
        <div className="flex gap-2 items-center">
          <button
            onClick={() => fetchFinances(true)}
            disabled={refreshing}
            className="text-sm ds-text-secondary hover:ds-text-primary disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-sm ds-text-secondary">Capital Raised</p>
          ...
          <div>
            <p className="text-sm ds-text-secondary">Capital Used</p>
            <p className="text-xl font-bold text-red-600 mt-1">
              {formatCurrency(finances.totalUsed || 0)}
            </p>
          </div>
          <div>
            <p className="text-sm ds-text-secondary">Remaining Balance</p>
        ...
        <p className="text-sm ds-text-muted">No financing data available</p>
      )}
    </div>
  );
}
    ...
    <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="text-center p-4 bg-blue-50 rounded-lg">
          ...
          <p className="text-sm ds-text-secondary mt-1">Photos</p>
        </div>
        <div className="text-center p-4 bg-green-50 rounded-lg">
          <p className="text-2xl font-bold text-green-600">{milestonesCount}</p>
          <p className="text-sm ds-text-secondary mt-1">Milestones</p>
        </div>
        <div className="text-center p-4 bg-purple-50 rounded-lg">
          <p className="text-2xl font-bold text-purple-600">{updatesCount}</p>
          <p className="text-sm ds-text-secondary mt-1">Daily Updates</p>
        </div>
      </div>
      <div className="mt-4">
        <Link
          href={`/projects/${projectId}/progress`}
          className="inline-flex items-center px-4 py-2 border ds-border-subtle rounded-md shadow-sm text-sm font-medium ds-text-secondary ds-bg-surface hover:ds-bg-surface-muted"
        >
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm sm:text-base ds-text-secondary mt-1 break-words">
              ...
              {canEdit && project.status !== 'archived' && (
                <>
                  ...
                  <Link
                    ...
                    className="flex-1 sm:flex-none px-4 py-2.5 ds-bg-surface-muted ds-text-secondary rounded-lg hover:ds-bg-surface-muted active:ds-bg-surface-muted transition-colors text-sm font-medium text-center touch-manipulation"
                  >
                    Cost Management
                  </Link>
                  <Link
        ...
        <div className="space-y-6 mt-8">
          ...
          <div className="ds-bg-surface rounded-lg shadow-sm p-4 sm:p-6 border ds-border-subtle">
            <h2 className="text-base sm:text-lg font-semibold ds-text-primary mb-4">Project Information</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <p className="text-sm ds-text-secondary">Project Code</p>
                <p className="text-base font-medium ds-text-primary mt-1">{project.projectCode}</p>
              </div>
              <div>
              ...
              {project.description && (
                <div className="sm:col-span-2">
                  ...
                  <p className="text-sm sm:text-base ds-text-primary mt-1 break-words">{project.description}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {!floorsLoading && floors.length === 0 && (
            <div className="ds-bg-surface rounded-lg shadow-sm p-4 sm:p-6 border ds-border-subtle">
              <div className="flex flex-col sm:flex-row items-start sm:justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h2 className="text-base sm:text-lg font-semibold ds-text-primary">Floors</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div>
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 text-base font-medium ds-text-primary ds-bg-surface border-2 ds-border-strong rounded-lg shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-300 focus:border-blue-600 transition-all placeholder:ds-text-muted placeholder:font-normal"
                />
              </div>
              <div>
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">
                ...
                <select
                  ...
                  onChange={handleChange}
                  required
                  className="w-full px-4 py-3 text-base font-medium ds-text-primary ds-bg-surface border-2 ds-border-strong rounded-lg shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-300 focus:border-blue-600 transition-all cursor-pointer"
                >
              ...
              <div>
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">Location</label>
                <input
                  type="text"
                  name="location"
              ...
              <div>
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">Client</label>
                <input
                  type="text"
                  name="client"
              ...
              <div>
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">Start Date</label>
                <div className="relative">
                  <input
                    type="date"
                    name="startDate"
                    ...
                    onChange={handleChange}
                    className="w-full px-4 pr-12 py-3 text-base font-medium ds-text-primary ds-bg-surface border-2 ds-border-strong rounded-lg shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-300 focus:border-blue-600 transition-all cursor-pointer"
                  />
                  ...
                  >
                    <svg
                      className="w-5 h-5 ds-text-secondary hover:text-blue-600 transition-colors"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
              ...
              <div>
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">Planned End Date</label>
                <div className="relative">
                  <input
                    type="date"
                    name="plannedEndDate"
              ...
              <div className="md:col-span-2">
                <label className="block text-base font-bold ds-text-primary mb-2 leading-tight">Description</label>
                <textarea
                  name="description"
                  ...
                  className="w-full px-4 py-3 text-base font-medium ds-text-primary ds-bg-surface border-2 ds-border-strong rounded-lg shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-300 focus:border-blue-600 transition-all resize-y placeholder:ds-text-muted placeholder:font-normal"
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 ds-bg-surface/80 border ds-border-subtle rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
              />
              ...
              {floorInitForm.includeBasements && (
                <div className="mt-4">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 ds-bg-surface/90 border ds-border-subtle rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                  />

  ...
  const fetchWorkItem = async () => {
    try {
      ...
      setFormData({
        ...
        executionModel: data.data.executionModel || '',
        subcontractorId: data.data.subcontractorId ? data.data.subcontractorId.toString() : ''
      });
      ...
      if (data.data.phaseId) {
        ...
        if (phaseData.success) {
          ...
          // Load subcontractors if this is a finishing phase
          if (phaseData.data.phaseType === 'finishing' && data.data.projectId && data.data.floorId) {
            setLoadingSubcontractors(true);
            try {
              const subsResponse = await fetch(
                `/api/subcontractors?projectId=${data.data.projectId}&floorId=${data.data.floorId}&status=active`,
                {
                  cache: 'no-store',
              ...
              if (subsData.success) {
                const subs = subsData.data?.subcontractors || subsData.data || [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold ds-text-primary break-words">{workItem.name}</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6">
          <div className="ds-bg-surface rounded-xl shadow-lg border ds-border-subtle p-6 hover:shadow-xl transition-shadow duration-200">
            ...
            <p className="text-sm font-semibold ds-text-secondary mt-6 mb-2 uppercase tracking-wide">Priority</p>
          ...
          <div className="ds-bg-surface rounded-xl shadow-lg border ds-border-subtle p-6 hover:shadow-xl transition-shadow duration-200">
            <p className="text-sm font-semibold ds-text-secondary mb-3 uppercase tracking-wide">Schedule</p>
        ...
        {workItem.description && (
          <div className="ds-bg-surface rounded-xl shadow-lg border ds-border-subtle p-4 sm:p-6 mb-6">
            ...
            <div className="ds-bg-surface-muted rounded-lg p-4 border ds-border-subtle">
              <p className="text-sm sm:text-base ds-text-primary whitespace-pre-wrap font-medium leading-relaxed break-words">{workItem.description}</p>
            </div>
          </div>
        )}
        ...
        {workItem.dependencies && workItem.dependencies.length > 0 && (
          <div className="ds-bg-surface rounded-xl shadow-lg border ds-border-subtle p-4 sm:p-6 mb-6">
            ...
            <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-400/60">
              <p className="text-xs sm:text-sm font-medium ds-text-primary">
        ...
        {workItem.assignedWorkers && workItem.assignedWorkers.length > 0 && (
          <div className="ds-bg-surface rounded-xl shadow-lg border ds-border-subtle p-4 sm:p-6 mb-6">
            ...
            {canAccess('create_labour_entry') && (
              <div className="mt-4 pt-4 border-t ds-border-subtle flex flex-col gap-2">
            ...
            <div className="ds-bg-surface rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border ds-border-subtle">
              <div className="p-4 sm:p-8">
                ...
                <form onSubmit={handleEditSubmit} className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-semibold ds-text-primary mb-2">
                      Description <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                    </label>
                    <textarea
                      name="description"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                      ...
                      {loadingCategories ? (
                        ...
                        >
                          ...
                          {categoryOptions.map((category) => (
                            <option key={category.value} value={category.value} className="ds-text-primary">
                      ...
                      {usesLegacyCategories && !loadingCategories && (
                        <p className="text-xs ds-text-secondary mt-1.5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-semibold ds-text-primary mb-2">
                        Actual Hours <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                      </label>
                      <div className="relative">
                        <span className="absolute right-4 top-1/2 transform -translate-y-1/2 ds-text-secondary font-medium">hrs</span>
                        <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                      <label className="block text-sm font-semibold ds-text-primary mb-2">
                        Estimated Cost (KES) <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                      </label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="block text-sm font-semibold ds-text-primary mb-2">
                        Actual Cost (KES) <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                      </label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 transform -translate-y-1/2 ds-text-secondary font-medium">KES</span>
                        <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                      <label className="block text-sm font-semibold ds-text-primary mb-2">
                        Start Date <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                      </label>
                      <input
                        type="date"
                        name="startDate"
                    ...
                    <div>
                      <label className="block text-sm font-semibold ds-text-primary mb-2">
                        Planned End Date <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                      </label>
                      <input
                        type="date"
                        name="plannedEndDate"
                  ...
                  {/* Finishing Works Fields - Only show for finishing phases */}
                  {phase?.phaseType === 'finishing' && (
                    <>
                      <div>
                        <label className="block text-sm font-semibold ds-text-primary mb-2">
                          Execution Model <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                        </label>
                        <select
                          name="executionModel"
                        ...
                        >
                          ...
                          {FINISHING_EXECUTION_MODELS.map((model) => (
                            <option key={model} value={model} className="ds-text-primary">
                              {model === 'direct_labour' ? 'Direct Labour' : 'Contract-Based'}
                            </option>
                          ))}
                        </select>
                      </div>
                      ...
                      {formData.executionModel === 'contract_based' && (
                        <div>
                          <label className="block text-sm font-semibold ds-text-primary mb-2">
                            Subcontractor <span className="ds-text-muted text-xs font-normal">(Optional)</span>
                          </label>
                          {loadingSubcontractors ? (
                            ...
                            >
                              ...
                              {subcontractors.map((sub) => (
                                <option key={sub._id} value={sub._id.toString()} className="ds-text-primary">

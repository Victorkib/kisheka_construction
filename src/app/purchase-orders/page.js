...
function PurchaseOrdersPageContent() {
  ...
  const getFinancialStatusBadgeColor = (financialStatus) => {
    const colors = {
      not_committed: 'ds-bg-surface-muted ds-text-primary',
      committed: 'bg-orange-100 text-orange-800',
      fulfilled: 'bg-green-100 text-green-800',
      cancelled: 'bg-red-100 text-red-800',
    };
    return colors[financialStatus] || 'ds-bg-surface-muted ds-text-primary';
  };
  ...
  if (isEmpty && !loading) {
    return (
      <AppLayout>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8">
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold ds-text-primary leading-tight">Purchase Orders</h1>
            <p className="text-base md:text-lg ds-text-secondary mt-2 leading-relaxed">Manage purchase orders and supplier interactions</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            {canAccess('view_purchase_orders') && (
              <div>
                <label className="block text-base font-semibold ds-text-secondary mb-1 leading-normal">Supplier</label>
                <select
        ...
        {orders.length === 0 && !loading ? (
          <div className="ds-bg-surface rounded-lg shadow p-12 text-center">
            <p className="text-lg ds-text-secondary mb-4">No purchase orders found</p>
            {canAccess('create_purchase_order') && (
              <Link
                href="/purchase-orders/new"
              ...
              <table className="min-w-full divide-y divide-ds-border-subtle">
                ...
                <tbody className="ds-bg-surface divide-y divide-ds-border-subtle">
                  {orders.map((order, index) => {
                    ...
                    return (
                      <tr key={order._id?.toString() || order.id?.toString() || `order-${index}`} className="hover:ds-bg-surface-muted">
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm font-medium ds-text-primary">{order.materialName}</div>
                          {order.description && (
                            <div className="text-sm ds-text-secondary truncate max-w-xs">{order.description}</div>
                          )}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm ds-text-primary">
                          {order.phaseName ? (
                            ...
                            <span className="ds-text-muted italic">Phase ID: {order.phaseId.toString().substring(0, 8)}...</span>
                          ) : (

...
import { createLabourBatch, validateLabourBatch, generateBatchNumber } from '@/lib/schemas/labour-batch-schema';
...
export async function POST(request) {
  try {
    ...
    if (!hasAccess) {
      return errorResponse('Insufficient permissions. You do not have permission to create labour batches.', 403);
    }
    ...
    if (!labourEntries || !Array.isArray(labourEntries) || labourEntries.length === 0) {
    ...
    if (!batchValidation.isValid) {
      return errorResponse(`Validation failed: ${batchValidation.errors.join(', ')}`, 400);
    }
    ...
    if (workItemId && ObjectId.isValid(workItemId)) {
      if (!defaultPhaseId || !ObjectId.isValid(defaultPhaseId)) {
        return errorResponse('Phase is required when linking to a work item', 400);
      }
      ...
      if (!workItem) {
        return errorResponse('Work item not found or does not belong to the selected phase', 404);
      }
    }
    ...
    for (const entryData of labourEntries) {
      ...
      const entryWithDefaults = {
        ...entryData,
        projectId,
        phaseId: entryData.phaseId || defaultPhaseId,
      ...
      if ((!entryData.workerId || entryData.workerId.trim() === '') && 
          entryData.workerName && entryData.workerName.trim().length >= 2 &&
    ...
    // Group entries by phaseId for budget validation
    ...
    for (const [phaseId, phaseEntries] of Object.entries(entriesByPhase)) {
      const phaseCost = phaseEntries.reduce((sum, entry) => sum + entry.totalCost, 0);
      const budgetValidation = await validatePhaseLabourBudget(phaseId, phaseCost);

      if (!budgetValidation.isValid) {
        return errorResponse(
          `Budget validation failed for phase: ${budgetValidation.message}`,
          400
        );
      }
    }
    ...
    const batchNumber = await generateBatchNumber(defaultDate ? new Date(defaultDate) : new Date());
    ...
    const batchData = {
      ...
      workItemId: workItemId || null, // Store work item ID at batch level
      labourEntries: validatedEntries,
      status: autoApprove ? 'approved' : 'draft',
    };

    const labourBatch = createLabourBatch(batchData, userProfile._id, `${userProfile.firstName || ''} ${userProfile.lastName || ''}`.trim() || userProfile.email);
    ...
    console.log('[POST /api/labour/batches] Starting transaction for atomic batch creation');
    ...
    const transactionResult = await withTransaction(async ({ db: transactionDb, session }) => {
        ...
        for (const [workerKey, entryData] of workerProfileDataMap.entries()) {
          try {
            ...
            if (workerProfile.createdAt && 
              ...
              console.log('[POST /api/labour/batches] Worker profile created:', workerProfile._id);
            }
          } catch (error) {
            console.error('[POST /api/labour/batches] Error creating worker profile:', error);
      ...
      console.log('[POST /api/labour/batches] Batch inserted:', batchResult.insertedId);
      ...
      console.log('[POST /api/labour/batches] Entries inserted:', entryIds.length);
      ...
      // STEP 4: Update phase budgets (atomic) - grouped by phase
      for (const [phaseId, phaseEntries] of Object.entries(entriesByPhase)) {
        const phaseCost = phaseEntries.reduce((sum, entry) => sum + entry.totalCost, 0);
        if (autoApprove) {
          await updatePhaseLabourSpending(phaseId, phaseCost, 'add', session);
        }
      }

      // STEP 5: Update project budget (atomic)
      if (autoApprove) {
        await updateProjectLabourSpending(projectId, totalBatchCost, 'add', session);
      }

      // STEP 6: Update work items if linked (atomic)
      ...
      await createAuditLog(
        {
          ...
          changes: {
            ...
            phaseBudgetImpacts: Object.entries(entriesByPhase).map(([phaseId, entries]) => ({
              phaseId,
    ...
    console.log('[POST /api/labour/batches] Transaction completed successfully');
    ...
    const successMessage = transactionResult.workerProfilesCreated > 0
      ? `Labour batch created and ${autoApprove ? 'approved' : 'saved as draft'} successfully. ${transactionResult.workerProfilesCreated} worker profile(s) created.`
      : `Labour batch created and ${autoApprove ? 'approved' : 'saved as draft'} successfully`;
    ...
    return successResponse(
      {
        ...
        budgetValidation: {
          totalCost: transactionResult.totalCost,
          phases: Object.entries(entriesByPhase).map(([phaseId, phaseEntries]) => ({
            phaseId,
    ...
    return errorResponse(
      error.message || 'Failed to create labour batch',
      500
    );
  }
}

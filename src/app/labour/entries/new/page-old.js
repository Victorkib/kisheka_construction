...
import { EquipmentOperatorInfo } from '@/components/labour/EquipmentOperatorInfo';
...
function NewLabourEntryPageContent() {
  ...
  useEffect(() => {
    ...
    // New: Equipment operation parameters
    const equipmentIdFromUrl = searchParams.get('equipmentId');
    const hourlyRateFromUrl = searchParams.get('hourlyRate');
    const dailyRateFromUrl = searchParams.get('dailyRate');
    const taskDescFromUrl = searchParams.get('taskDescription');
    ...
    // Auto-fill equipment operation data
    if (equipmentIdFromUrl) {
      setFormData((prev) => ({ 
        ...prev, 
        equipmentId: equipmentIdFromUrl,
        taskDescription: taskDescFromUrl || prev.taskDescription,
      }));
    }

    if (hourlyRateFromUrl) {
      setFormData((prev) => ({ ...prev, hourlyRate: hourlyRateFromUrl }));
    }

    if (dailyRateFromUrl) {
      setFormData((prev) => ({ ...prev, dailyRate: dailyRateFromUrl }));
    }
  }, [searchParams]);
          ...
          <div className="space-y-6">
            ...
            <div className="border-t pt-6">
              ...
              <p className="text-sm ds-text-secondary mb-4">
                If you operated equipment during this work, link it here. This helps track equipment utilization and operator costs.
              </p>
              <EquipmentOperatorInfo

...
export default function ApprovalsPage() {
  ...
  const fetchPendingApprovals = useCallback(async () => {
    ...
    try {
      ...
      const materialsResponse = await fetch(
        `/api/materials?status=pending_approval&limit=100${projectIdParam}`,
      );
      const materialsData = await materialsResponse.json();

      if (!materialsData.success) {
        throw new Error(
          materialsData.error || 'Failed to fetch pending material approvals',
        );
      }
      ...
      const expensesResponse = await fetch(
        `/api/expenses?status=PENDING&limit=100${projectIdParam}`,
      );
      const expensesData = await expensesResponse.json();

      if (!expensesData.success) {
        throw new Error(
          expensesData.error || 'Failed to fetch pending expense approvals',
        );
      }
      ...
      const initialExpensesResponse = await fetch(
        `/api/initial-expenses?status=pending_approval&limit=100${projectIdParam}`,
      );
  ...
  const handleBulkApproveMaterials = async () => {
    ...
    try {
      ...
      toast.showSuccess(
        `Approved ${selectedMaterials.length} material(s) successfully!`,
      );
    } catch (err) {
      toast.showError(`Error: ${err.message}`);
    } finally {
  ...
  const handleBulkRejectMaterials = async () => {
    ...
    try {
      ...
      setSelectedMaterials([]);
      toast.showSuccess(
        `Rejected ${selectedMaterials.length} material(s) successfully!`,
      );
    } catch (err) {
      toast.showError(`Error: ${err.message}`);
    } finally {
  ...
  const handleBulkApproveExpenses = async () => {
    ...
    try {
      ...
      setSelectedExpenses([]);
      toast.showSuccess(
        `Approved ${selectedExpenses.length} expense(s) successfully!`,
      );
    } catch (err) {
      toast.showError(`Error: ${err.message}`);
    } finally {
  ...
  const handleBulkRejectExpenses = async () => {
    ...
    try {
      ...
      setSelectedExpenses([]);
      toast.showSuccess(
        `Rejected ${selectedExpenses.length} expense(s) successfully!`,
      );
    } catch (err) {
      toast.showError(`Error: ${err.message}`);
    } finally {
  ...
  const handleBulkApproveInitialExpenses = async () => {
    ...
    try {
      ...
      setSelectedInitialExpenses([]);
      toast.showSuccess(
        `Approved ${selectedInitialExpenses.length} initial expense(s) successfully!`,
      );
    } catch (err) {
      toast.showError(`Error: ${err.message}`);
    } finally {
  ...
  const handleBulkRejectInitialExpenses = async () => {
    ...
    try {
      ...
      setSelectedInitialExpenses([]);
      toast.showSuccess(
        `Rejected ${selectedInitialExpenses.length} initial expense(s) successfully!`,
      );
    } catch (err) {
      toast.showError(`Error: ${err.message}`);
    } finally {
          ...
          <>
            ...
            {!loading && materials.length > 0 && (
              <div className="mb-6 space-y-3">
                {Array.from(
                  new Set(materials.map((m) => m.projectId).filter(Boolean)),
                ).map((projectId) => {
        ...
        {activeTab === 'expenses' && (
          <>
            ...
            {!loading && expenses.length > 0 && (
              <div className="mb-6 space-y-3">
                {Array.from(
                  new Set(expenses.map((e) => e.projectId).filter(Boolean)),
                ).map((projectId) => {
          ...
          <>
            ...
            {!loading && initialExpenses.length > 0 && (
              <div className="mb-6 space-y-3">
                {Array.from(
                  new Set(
                    initialExpenses.map((e) => e.projectId).filter(Boolean),
                  ),
                ).map((projectId) => {
                  const projectInitialExpenses = initialExpenses.filter(
                    (e) => e.projectId === projectId,
                  );

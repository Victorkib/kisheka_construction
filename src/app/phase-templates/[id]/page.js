      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold ds-text-primary">{template.templateName}</h1>
        ...
        {template.description && (
          <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
            <h2 className="text-lg font-semibold ds-text-primary mb-2">Description</h2>
            <p className="ds-text-secondary">{template.description}</p>
          </div>
        )}
        ...
        <div className="ds-bg-surface rounded-lg shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div>
              ...
              <p className="text-xl font-bold ds-text-primary">{template.defaultBudgetAllocation?.materials || 0}%</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Labour</p>
              <p className="text-xl font-bold ds-text-primary">{template.defaultBudgetAllocation?.labour || 0}%</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Equipment</p>
              <p className="text-xl font-bold ds-text-primary">{template.defaultBudgetAllocation?.equipment || 0}%</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Subcontractors</p>
              <p className="text-xl font-bold ds-text-primary">{template.defaultBudgetAllocation?.subcontractors || 0}%</p>
            </div>
            <div>
              <p className="text-sm ds-text-secondary">Contingency</p>
              <p className="text-xl font-bold ds-text-primary">{template.defaultBudgetAllocation?.contingency || 0}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="ds-bg-surface rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold ds-text-primary mb-4">Phase Definitions ({template.phases?.length || 0})</h2>
          <div className="space-y-4">
            {template.phases && template.phases.length > 0 ? (
              template.phases.map((phase, index) => (
                <div key={index} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="text-md font-semibold ds-text-primary">
                        {phase.phaseCode}: {phase.phaseName}
                      </h3>
                      <p className="text-sm ds-text-secondary capitalize mt-1">{phase.phaseType?.replace('_', ' ')}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm ds-text-secondary">Budget</p>
                      <p className="text-lg font-bold ds-text-primary">{phase.defaultBudgetPercentage}%</p>
                    </div>
                  </div>
                  {phase.description && (
                    <p className="text-sm ds-text-secondary mb-3">{phase.description}</p>
                  )}
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {phase.defaultWorkItems && phase.defaultWorkItems.length > 0 && (
                      <div>
                        <p className="ds-text-secondary font-medium mb-1">Default Work Items:</p>
                        <ul className="list-disc list-inside ds-text-secondary">
                          {phase.defaultWorkItems.slice(0, 5).map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                          {phase.defaultWorkItems.length > 5 && (
                            <li className="ds-text-muted">+{phase.defaultWorkItems.length - 5} more</li>
                          )}
                        </ul>
                      </div>
                    )}
                    {phase.defaultMilestones && phase.defaultMilestones.length > 0 && (
                      <div>
                        <p className="ds-text-secondary font-medium mb-1">Default Milestones:</p>
                        <ul className="list-disc list-inside ds-text-secondary">
                          {phase.defaultMilestones.slice(0, 5).map((milestone, idx) => (
                            <li key={idx}>{milestone}</li>
                          ))}
                          {phase.defaultMilestones.length > 5 && (
                            <li className="ds-text-muted">+{phase.defaultMilestones.length - 5} more</li>
                          )}
                        </ul>
                      </div>
                    )}
              ...
              <p className="ds-text-muted text-sm">No phases defined in this template</p>
            )}
          </div>
        </div>
            ...
            <div className="ds-bg-surface rounded-lg shadow-xl max-w-md w-full mx-4">
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold ds-text-primary">Apply Template to Project</h3>
                  <button
                ...
                {isEmpty ? (
                  ...
                  <>
                    <div className="mb-4">
                      <label htmlFor="project-select" className="block text-sm font-medium ds-text-secondary mb-2">
